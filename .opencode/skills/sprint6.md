...
- **Caja**: usuario (Cajero), fecha_apertura, fecha_cierre, estado (ABIERTA, CERRADA, CERRADA_CON_DESCUADRE).
- **BalanceCajaDivisa**: Sostenimiento multimoneda del saldo por caja (saldo_inicial, saldo_actual, saldo_cierre).
- **ArqueoCaja**: caja, 	ipo (APERTURA, PARCIAL, CIERRE), 	otal_calculado, 	otal_real, diferencia, faltante, descontar_sueldo (boolean), observaciones.
...
- **RegistroAuditoria**: Cifrado/JSON con usuario, accion_realizada, entidad_afectada, datos_anteriores, datos_nuevos, 	imestamp.
